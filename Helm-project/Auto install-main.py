...
render_run = project_dir + "Custom-value-complete/run/%s/%s/"%(env,chart_dir) + os.path.basename(run_file_tem)
if mod_nginx == "--mod-nginx":
...
def install():
    ...
    # os.chdir(root_app)   s
    run_dir = project_dir +  "Custom-value-template/%s/%s/"%(env,chart_dir) + "run"
    ...
    time.sleep(0.5)
    # subprocess.call("helm " + action + " " + chart_dir + " " + project_dir + "Helm-app/%s/ -f "%chart_dir + value_file)
def uninstall():
        print("Executing command: helm uninstall %s"%release_name )
        # subprocess.call("helm uninstall %s"%chart_dir)
if action == "uninstall":
    uninstall()
else:
    install()